...
const container = document.getElementById("container"); // Container que contém osblocos dos produtos
const burger = document.getElementById("burger"); // Ícone de menu (hambúrguer) paraabrir o menu lateral
const menu = document.getElementById("menu"); // O menu lateral que se expande aoclicar no ícone
const cartIcon = document.getElementById("cart-icon"); // Ícone do carrinho de compras
const cartSidebar = document.getElementById("cart-sidebar"); // Sidebar que mostra oconteúdo do carrinho
const cartItemsContainer = document.getElementById("cart-items"); // Contêiner ondeserão listados os itens adicionados ao carrinho
const finalizarBtn = document.getElementById("finalizar-btn"); // Botão para finalizaro pedido
...
function renderCart() {
    ...
    cart.forEach((item) => {
        ...
        const totalPrice = (item.price * item.quantity).toFixed(2); // Calcula o preçototal do item (quantidade * preço unitário)
        cartItem.innerHTML = `
...
Array.from({ length: 6 }).map((_, i) => {
    ...
    <img
    src='https://polosulsc.com.br/wp-content/webp-express/webp-images/uploads/2018/03/Tr%C3%AAs-tipos-e-sabores-de-sorvete-diferentes-1.jpg.webp' alt='Sorvete'
class="sorvetinho">
   </div>
...
function changeImage() {
    ...
    images.forEach((image) => {
        if (window.innerWidth <= 677) {
            ...
            image.src = "https://gkpb.com.br/wp-content/uploads/2022/10/novo-bobs-candy-dia-das-criancas.jpg";
            // Define uma imagem diferente (apenas para exemplo)
        } else {
...
function renderCart() {
    ...
    // Para cada item no carrinho, cria um elemento visual e o adiciona ao container do carrinho
    cart.forEach((item) => {
        const cartItem = document.createElement("div"); // Cria um div para representar umitem
        cartItem.className = "cart-item"; // Define a classe CSS para o item do carrinho
        const totalPrice = (item.price * item.quantity).toFixed(2); // Calcula o preço total do item (quantidade * preço unitário)
        cartItem.innerHTML = `
    <h4>${item.quantity}x ${item.name}</h4> <!-- Exibe a quantidade e o nome do
    item -->
    <p>Preço: R$ ${totalPrice}</p> <!-- Exibe o preço total do item -->
